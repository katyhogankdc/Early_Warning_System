IF OBJECT_ID('CUSTOM.CUSTOM_EARLY_WARNING_FACT', 'U') IS NOT NULL
...
IF (SELECT OBJECT_ID
	FROM sys.indexes 
	WHERE name='EW_FACT' AND object_id = OBJECT_ID('CUSTOM.CUSTOM_EARLY_WARNING_FACT')) IS NULL
	CREATE INDEX EW_FACT ON CUSTOM.CUSTOM_EARLY_WARNING_FACT (STUDENT_NUMBER,TERMKEY,STUDENTKEY,STUDENTID);

INSERT INTO CUSTOM.CUSTOM_EARLY_WARNING_FACT 
SELECT
A.STUDENT_NUMBER,
A.STUDENTID,
A.STUDENTKEY,
...
LEFT JOIN [CUSTOM].[CUSTOM_EARLY_WARNING_FP] FP ON FP.STUDENT_NUMBER = A.STUDENT_NUMBER AND FP.TERMKEY = A.TERMKEY
LEFT JOIN [custom].[CUSTOM_EARLY_WARNING_RIT_TO_READING] R ON R.STUDENT_NUMBER = A.STUDENT_NUMBER AND R.TERMKEY = A.TERMKEY
