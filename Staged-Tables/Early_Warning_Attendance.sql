...
CREATE INDEX EW_GRADES ON CUSTOM.CUSTOM_EARLY_WARNING_ATTENDANCE (STUDENTKEY,TERMKEY);
...
SUM(1) MEMBERSHIP,
SUM(CASE WHEN DESCRIPTION IN ('Absent','Medical Unexcused','Tardy Absent','Released Early Absent') THEN 1 ELSE 0 END) UNEXCUSED_ABSENCES, --there is ap probably a better value to pull unexcused absences from so they don't need be hardcoded
SUM(CASE WHEN DESCRIPTION IN ('Tardy','Tardy Excused','Tardy Released Early') THEN 1 ELSE 0 END) TARDIES --find a better field to pull tardies from instead of hard coding
--INTO #EW_Attendance
