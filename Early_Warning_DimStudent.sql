...
IF OBJECT_ID('CUSTOM.CUSTOM_EARLY_WARNING_STUDENTS', 'U') IS NOT NULL
	DELETE FROM CUSTOM.CUSTOM_EARLY_WARNING_STUDENTS
ELSE
...
IF (SELECT OBJECT_ID
	FROM sys.indexes 
	WHERE name='EW_STUDENTS' AND object_id = OBJECT_ID('CUSTOM.CUSTOM_EARLY_WARNING_STUDENTS')) IS NULL
...
PS.GENDER,
CASE WHEN PS.ENTRYCODE NOT IN ('PR','RE','DE','KIPP') THEN 1 ELSE 0 END AS NEW_TO_KIPP
